...
function LAUNCH_STREAM() {
	# --nodelist="${USE_NODES}"
	sbatch -p "test" -O --output="${OUTPUT1}" --open-mode=append -C "nomonitoring" -N "$1" --ntasks-per-node="$2" ${PATH_TO_DIR}/sbatch_files/my_sbatch_x1 ${PATH_TO_EXEC}/${EXEC_FILE} ${OUTPUT1} ${5} ${OUTPUT2} ${6} 
}
