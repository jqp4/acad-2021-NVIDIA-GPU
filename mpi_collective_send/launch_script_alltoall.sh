...
function LAUNCH_STREAM() {
	sbatch -p "test" -O --output="${OUTPUT1}" --nodelist="${USE_NODES}" --open-mode=append -C "nomonitoring" -N "$1" --ntasks-per-node="$2" ${PATH_TO_DIR}/sbatch_files/my_sbatch_clean ${PATH_TO_EXEC}/${EXEC_FILE} ${OUTPUT1} ${5} ${6} ${OUTPUT2} ${7} 
}
